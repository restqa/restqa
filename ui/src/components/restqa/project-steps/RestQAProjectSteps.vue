  ...
  <card :title="title" :loading="null === data">
    <el-table :data="data" class="step-definition" style="width: 100%">
      <el-table-column class="expand" type="expand">
        <template #default="props">
          <el-descriptions title="Detail" :column="1" border>
            ...
            <el-descriptions-item label="Plugin" class="plugin">{{
              props.row.plugin
            }}</el-descriptions-item>
          </el-descriptions>
        </template>
      ...
      <el-table-column fixed="right" label="Operations" width="120">
        <template #default="props">
          <el-button type="text" @click="copy(props.row)" size="small"
            >Copy</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  computed: {
    title() {
      const suffix =
        (this.data && ` - ${this.data.length} step definitions`) || "";
      return `${
        this.keyword[0].toUpperCase() + this.keyword.substring(1)
      }${suffix}`;
    }
  },
  methods: {
