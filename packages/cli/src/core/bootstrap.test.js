...
describe("#bootstrap", () => {
  ...
  test("Load plugin @restqa/rest-api-plugin then run setup the processor", () => {
    const content = `
---
version: 0.0.1
metadata:
...
`;
    ...
    expect(world.getConfig("rest-api-plugin")).toEqual({
      url: "http://localhost:8088"
    });
  ...
  test("Load plugin @restqa/rest-api-plugin then run setup the processor (integration tests)", () => {
    const content = `
---
version: 0.0.1
metadata:
...
`;
  ...
  test("Load plugin rest-api-plugin and restqkube then run setup the processor (+ setup the timeout)", () => {
    const content = `
---
version: 0.0.1
metadata:
...
settings:
  timeout: 10000
    ...
    jest.mock("@restqa/rest-api-plugin", () => mockPlugins[0]);
