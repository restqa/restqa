...
describe('# utils - telemetry - index', () => {
  describe('# utils - telemetry - toogle', () => {
    ...
    test('Create telemetry and tooble on / off the consent', () => {
      ...
      expect(JSON.parse(fs.readFileSync(filename).toString('utf-8'))).toEqual({ telemetry: true })

      telemetry.toggle(false)

      expect(JSON.parse(fs.readFileSync(filename).toString('utf-8'))).toEqual({ telemetry: false })
    })
  })
