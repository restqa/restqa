...
describe("RestQAProjectEditorRunnerStep", () => {
  ...
  test("Show a success step", async () => {
    const data = {
      ...
      match: {
        location:
          "../node_modules/@restqa/restqapi/src/restqapi/steps/index.js:6"
      },
      result: {
        status: "passed",
        duration: 0
  ...
  test("Show a failed step", async () => {
    const data = {
      ...
      match: {
        location:
          "../node_modules/@restqa/restqapi/src/restqapi/steps/index.js:16"
      },
      result: {
        status: "failed",
        duration: 1000000,
        error_message:
          "AssertionError [ERR_ASSERTION]: [POST /todos] The response body property title should be sssA new RestQA post <string> but received A new RestQA post <string>\n    + expected - actual\n\n    -A new RestQA post\n    +sssA new RestQA post\n\n    at RestQA.Then.shouldBeString (/Users/olivierodo/WORKS/restqa/restqa/node_modules/@restqa/restqapi/src/restqapi/steps/3-then/functions.js:116:14)"
      }
    };
    ...
    expect(component.find(".step").text()).toBe(
      'the response body at "title" should equal "sssA new RestQA post"'
    );
    const btnError = component.findComponent(".btn-error");
  ...
  test("Show step attachement (json)", async () => {
    ...
    expect(component.find(".step").text()).toBe(
      "I should receive a response with the status 200"
    );
    const btnError = component.findComponent(".btn-error");
    expect(btnError.exists()).toBe(false);
    ...
    expect(debug.exists()).toBe(true);
    expect(debug.isVisible()).toBe(true);
    expect(debug.text()).toEqual(
      JSON.stringify(JSON.parse(data.embeddings[0].data), null, 2)
    );
  ...
  test("Show multiple step attachement (json + txt)", async () => {
    ...
    expect(debug[1].text()).toEqual(
      JSON.stringify(JSON.parse(data.embeddings[1].data), null, 2)
    );
