...
function transform (result, api) {
  ...
  try {
    ...
    path[endpoint.getPath()].addMethod(new Method(api)).addResponse(new Response(api.response))
    if (String(api.response.statusCode)[0] === '2') {
      ...
      for (const [key, example] of Object.entries(qs)) {
        path[endpoint.getPath()].getMethod(api.request.method).setParameters(key, 'query', example)
      }

      for (const [key, example] of Object.entries(api.request.headers || {})) {
        path[endpoint.getPath()].getMethod(api.request.method).setParameters(key, 'header', example)
      }

      path[endpoint.getPath()].getMethod(api.request.method).setRequestBody(api.request)
    }
  } catch(e) {
...
function Endpoint (api) {
  ...
  function addMethod (method) {
     ...
     swag.parameters.forEach(name => methods[method.name].setParameters(name, 'path'))
...
function Method ({ scenario, request, response }) {
  ...
  function toJSON () {
    const obj = {
      ...
      summary: scenario.pickle.name,
      parameters: parameters.sort((a, b) =>  a.type.localeCompare(b.type)).map(_ => _.toJSON()),
      requestBody,
