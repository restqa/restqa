...
async function Install(name, program) {
  ...
  let {config, env} = program;

  if (name && !LIST[name]) {
    throw new Error(
      `The plugin "${name}" is not available. Use the command "restqa install" to retrive the list of available plugin`
    );
  }
      ...
      await inquirer.prompt([
        {
          ...
          name: "name",
          choices: Object.values(filteredList)
            .map((list) => {
  ...
  logger.success(
    "service.install.success.config_updated",
    name,
    LIST[name].type
  );
...
Install.generate = function (options) {
  ...
  const {name, env, configFile, config} = options;
  ...
  if (envIndex === -1) {
    throw new TypeError(
      `"${env}" is not an environment available in the config file, choose between : ${restqaConfig.environments
        .map((_) => _.name)
        .join(", ")}`
    );
  }
  ...
  if (plugin.type === "outputs") {
    restqaConfig.environments[envIndex].outputs =
      restqaConfig.environments[envIndex].outputs || [];
